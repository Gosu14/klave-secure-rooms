...
const printClusterInfo = (): void => {
    ...
    console.info(
        'Klave Secure Rooms App now using the following cluster configuration:'
    );
    ...
    initialize: (): void => {
        ...
        fetch(
            ...
            .then((config: any) => {
                ...
                console.info('Klave Secure Rooms App now using config.json overrides');
            })
            .catch(() => {
                processClusterConfig(clusterConfigBase);
            });
    ...
    connect: (): Promise<ConnectionInformation> =>
        new Promise((resolve, reject) => {
            ...
            if (cluster && endpoint && handlerStore.currentKey) {
                console.info(
                    'Klave Secure Rooms App now using the following gateway:',
                    endpoint
                );
